...
async function publishCloudShell() {
  console.log("=========================================================");
  console.log("☁️ GOOGLE CLOUD SHELL AUTOMATED BLOG PUBLISHER ☁️");
  console.log("=========================================================");

  let accessToken = "";
  try {
    console.log("\n🔑 Fetching OAuth Access Token from Google Cloud Shell (`gcloud auth print-access-token`)...");
    accessToken = execSync("gcloud auth print-access-token").toString().trim();
    console.log("✅ Successfully acquired cloud admin token!");
  } catch (err) {
    console.error("❌ Failed to get access token from gcloud. Make sure you are running inside Google Cloud Shell.");
    process.exit(1);
  }

  const nowIso = new Date().toISOString();

  console.log(`\n📚 Uploading ${blogs.length} God-Level SEO Blogs directly via Firestore REST API...`);

  for (let i = 0; i < blogs.length; i++) {
    const post = blogs[i];
    process.stdout.write(`   [${i + 1}/${blogs.length}] Pushing "${post.title.substring(0, 45)}..." `);
    ...
    const url = `https://firestore.googleapis.com/v1/projects/${PROJECT_ID}/databases/(default)/documents/insights/${post.slug}`;
  ...
  console.log("\n🌍 Updating Google Sitemap (public/sitemap.xml)...");
  try {
    execSync("node scripts/generate-sitemap.js", { stdio: "inherit" });
    console.log("\n🎉 ALL 10 BLOGS PUBLISHED IN CLOUD SHELL ON AUTOPILOT! 🚀");
  } catch (err) {
    console.error("⚠️ Note: Sitemap generation failed or done locally.");
  }
}
